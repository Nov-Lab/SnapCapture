// @(h)NLSCAppBase.cs ver 0.00 ( '23.04.18 Nov-Lab ) 作成開始
// @(h)NLSCAppBase.cs ver 1.01 ( '23.09.14 Nov-Lab ) 初版完成
// @(h)NLSCAppBase.cs ver 1.01a( '23.09.30 Nov-Lab ) その他  ：コメント整理

// @(s)
// 　【SnapCapture・メインモジュール】アプリケーション全体で使うクラス・定数・列挙定数などを定義します。
...
namespace SnapCapture
{
    ...
    public class ImageFormatInfo
    {
        ...
        /// <summary>
        /// 【種別からイメージ形式情報を取得】
        /// 選択肢テーブルの中から、イメージ形式種別に合致するイメージ形式情報を取得します。
        /// </summary>
        ...
        {
            //------------------------------------------------------------
            /// 選択肢テーブルの中から、イメージ形式種別に合致するイメージ形式情報を取得する
            //------------------------------------------------------------
            foreach (var item in options)
            {                                                           //// 選択肢テーブルを繰り返す
                if (item.Kind == kind)
                {                                                       /////  イメージ形式種別が合致する場合
                    return item;                                        //////   戻り値 = イメージ形式情報 で関数終了
                }
            }

            return null;                                                //// 合致するものが見つからなかった場合、戻り値 = null(該当なし) で関数終了
        }
